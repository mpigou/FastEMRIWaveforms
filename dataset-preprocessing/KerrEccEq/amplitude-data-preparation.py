...
# mask for m == 0
m0mask = np.array(
    [m == 0 for l in range(2, 11) for m in range(0, l + 1) for n in range(-55, 56)]
)
...
with h5py.File("ZNAmps_l10_m10_n55_DS2Outer.h5", "w") as f:
    ...
    coeff_arr_out = regionA.create_dataset(
        "CoeffsRegionA", (33, l_arr_no_mask.size, 2, 33 * 33), dtype="f8"
    )
    ...
    for k, (ell_select, emm_select, enn_select) in tqdm(
        enumerate(zip(l_arr_no_mask, m_arr_no_mask, n_arr_no_mask))
    ):
        if enn_select >= 0:
            ...
            mode_path = os.path.join(
                ...
                f"l{ell_select}",
                f"m{emm_select}",
                f"amplitudes_{ell_select}_{emm_select}_{enn_select}.feather",
            )
        ...
        for spin_ind in range(33):
            spl1 = RectBivariateSpline(
                w_knots_inner, u_knots_inner, mode[:, :, spin_ind, 0].T, kx=3, ky=3
            )
            spl2 = RectBivariateSpline(
                w_knots_inner, u_knots_inner, mode[:, :, spin_ind, 1].T, kx=3, ky=3
            )
    ...
    coeff_arr_out_B = regionB.create_dataset(
        "CoeffsRegionB", (33, l_arr_no_mask.size, 2, 17 * 17), dtype="f8"
    )
    for k, (ell_select, emm_select, enn_select) in tqdm(
        enumerate(zip(l_arr_no_mask, m_arr_no_mask, n_arr_no_mask))
    ):
        ...
        for spin_ind in range(33):
            spl1 = RectBivariateSpline(
                w_knots_outer, u_knots_outer, mode[::2, ::2, spin_ind, 0].T, kx=3, ky=3
            )
            spl2 = RectBivariateSpline(
                w_knots_outer, u_knots_outer, mode[::2, ::2, spin_ind, 1].T, kx=3, ky=3
            )
    ...
    params = (
        paramsB.to_numpy().reshape(33, 33, 33, 3)[::2, ::2, :, :].transpose(2, 0, 1, 3)
    )
