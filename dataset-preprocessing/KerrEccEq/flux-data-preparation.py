...
with h5py.File("KerrEccEqFluxData.h5", "w") as outfile:
    ...
    regionA.create_dataset(
        "Edot", data=dataA["Edot"].to_numpy().reshape(NU, NW, NZ)[:, :, :]
    )
    regionA.create_dataset(
        "Ldot", data=dataA["Ldot"].to_numpy().reshape(NU, NW, NZ)[:, :, :]
    )
    ...
    regionB.create_dataset(
        "Edot", data=dataB["Edot"].to_numpy().reshape(NU, NW, NZ)[:, :, :]
    )
    regionB.create_dataset(
        "Ldot", data=dataB["Ldot"].to_numpy().reshape(NU, NW, NZ)[:, :, :]
    )
