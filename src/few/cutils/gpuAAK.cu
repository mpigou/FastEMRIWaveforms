...
double bessel_jn(int n, double x) {
  ...
  if (n == 0) return (bessel_j0(ax));
  if (n == 1) return (bessel_j1(ax));
...
CUDA_CALLABLE_MEMBER
void d_RotCoeff(double* rot, double* n, double* L, double* S, double* nxL,
                double* nxS, double iota, double theta_S, double phi_S,
                double theta_K, double phi_K, double alpha) {
  ...
  S[2] = cos(theta_K);
  L[0] = cos(iota) * sin(theta_K) * cos(phi_K) +
         sin(iota) *
             (sin(alpha) * sin(phi_K) - cos(alpha) * cos(theta_K) * cos(phi_K));
  L[1] = cos(iota) * sin(theta_K) * sin(phi_K) -
         sin(iota) *
             (sin(alpha) * cos(phi_K) + cos(alpha) * cos(theta_K) * sin(phi_K));
...
CUDA_KERNEL
void make_waveform(cmplx* waveform, double* interp_array, double M_phys,
                   double S_phys, double mu, double qS, double phiS, double qK,
                   double phiK, double dist, int nmodes, bool mich,
                   double delta_t, double start_t, int old_ind, int start_ind,
                   int end_ind, int init_length, double segwidth) {
#ifdef __CUDACC__
  ...
  CUDA_SHARED double Phi_phi_y, Phi_phi_c1, Phi_phi_c2, Phi_phi_c3, Phi_phi_c4,
      Phi_phi_c5, Phi_phi_c6, Phi_phi_c7;
  CUDA_SHARED double Phi_theta_y, Phi_theta_c1, Phi_theta_c2, Phi_theta_c3,
      Phi_theta_c4, Phi_theta_c5, Phi_theta_c6, Phi_theta_c7;
  CUDA_SHARED double Phi_r_y, Phi_r_c1, Phi_r_c2, Phi_r_c3, Phi_r_c4, Phi_r_c5,
      Phi_r_c6, Phi_r_c7;
...
#endif
  {
    e_y = spline_coeffs[1 * 8 + 0];
    e_c1 = spline_coeffs[1 * 8 + 1];
    e_c2 = spline_coeffs[1 * 8 + 2];
    e_c3 = spline_coeffs[1 * 8 + 3];
    e_c4 = spline_coeffs[1 * 8 + 4];
    e_c5 = spline_coeffs[1 * 8 + 5];
    e_c6 = spline_coeffs[1 * 8 + 6];
    e_c7 = spline_coeffs[1 * 8 + 7];
    Y_y = spline_coeffs[2 * 8 + 0];
    Y_c1 = spline_coeffs[2 * 8 + 1];
    Y_c2 = spline_coeffs[2 * 8 + 2];
    Y_c3 = spline_coeffs[2 * 8 + 3];
    Y_c4 = spline_coeffs[2 * 8 + 4];
    Y_c5 = spline_coeffs[2 * 8 + 5];
    Y_c6 = spline_coeffs[2 * 8 + 6];
    Y_c7 = spline_coeffs[2 * 8 + 7];
    Phi_phi_y = spline_coeffs[3 * 8 + 0];
    Phi_phi_c1 = spline_coeffs[3 * 8 + 1];
    Phi_phi_c2 = spline_coeffs[3 * 8 + 2];
    Phi_phi_c3 = spline_coeffs[3 * 8 + 3];
    Phi_phi_c4 = spline_coeffs[3 * 8 + 4];
    Phi_phi_c5 = spline_coeffs[3 * 8 + 5];
    Phi_phi_c6 = spline_coeffs[3 * 8 + 6];
    Phi_phi_c7 = spline_coeffs[3 * 8 + 7];
    Phi_theta_y = spline_coeffs[4 * 8 + 0];
    Phi_theta_c1 = spline_coeffs[4 * 8 + 1];
    Phi_theta_c2 = spline_coeffs[4 * 8 + 2];
    Phi_theta_c3 = spline_coeffs[4 * 8 + 3];
    Phi_theta_c4 = spline_coeffs[4 * 8 + 4];
    Phi_theta_c5 = spline_coeffs[4 * 8 + 5];
    Phi_theta_c6 = spline_coeffs[4 * 8 + 6];
    Phi_theta_c7 = spline_coeffs[4 * 8 + 7];
    Phi_r_y = spline_coeffs[5 * 8 + 0];
    Phi_r_c1 = spline_coeffs[5 * 8 + 1];
    Phi_r_c2 = spline_coeffs[5 * 8 + 2];
    Phi_r_c3 = spline_coeffs[5 * 8 + 3];
    Phi_r_c4 = spline_coeffs[5 * 8 + 4];
    Phi_r_c5 = spline_coeffs[5 * 8 + 5];
    Phi_r_c6 = spline_coeffs[5 * 8 + 6];
    Phi_r_c7 = spline_coeffs[5 * 8 + 7];
  }
  ...
  for (int i = start; i < end; i += increment) {
    ...
    double e =
        ...
        s * (e_c1 +
             s1 * (e_c2 +
                   s * (e_c3 +
                        s1 * (e_c4 + s * (e_c5 + s1 * (e_c6 + s * e_c7))))));
    double Y =
        Y_y +
        s * (Y_c1 +
             s1 * (Y_c2 +
                   s * (Y_c3 +
                        s1 * (Y_c4 + s * (Y_c5 + s1 * (Y_c6 + s * Y_c7))))));
        ...
        s * (Phi_theta_c1 +
             s1 * (Phi_theta_c2 +
                   s * (Phi_theta_c3 +
                        s1 * (Phi_theta_c4 +
                              s * (Phi_theta_c5 +
                                   s1 * (Phi_theta_c6 + s * Phi_theta_c7))))));
    double Phi_r =
        Phi_r_y +
        s * (Phi_r_c1 +
             s1 * (Phi_r_c2 +
    ...
    double OmegaPhi = (Phi_phi_c1 + Phi_phi_c2 * (1. - 2. * s) +
                       Phi_phi_c3 * (2. * s - 3. * s2) +
                       Phi_phi_c4 * (2. * s - 6. * s2 + 4. * s3) +
                       Phi_phi_c5 * (3. * s2 - 8. * s3 + 5. * s4) +
                       Phi_phi_c6 * (3. * s2 - 12. * s3 + 15 * s4 - 6. * s5) +
                       Phi_phi_c7 * (4. * s3 - 15. * s4 + 18. * s5 - 7. * s6)) /
                      segwidth;
    double OmegaTheta =
        (Phi_theta_c1 + Phi_theta_c2 * (1. - 2. * s) +
         Phi_theta_c3 * (2. * s - 3. * s2) +
         Phi_theta_c4 * (2. * s - 6. * s2 + 4. * s3) +
         Phi_theta_c5 * (3. * s2 - 8. * s3 + 5. * s4) +
         Phi_theta_c6 * (3. * s2 - 12. * s3 + 15 * s4 - 6. * s5) +
         Phi_theta_c7 * (4. * s3 - 15. * s4 + 18. * s5 - 7. * s6)) /
        segwidth;
    double OmegaR =
        (Phi_r_c1 + Phi_r_c2 * (1. - 2. * s) + Phi_r_c3 * (2. * s - 3. * s2) +
         Phi_r_c4 * (2. * s - 6. * s2 + 4. * s3) +
         Phi_r_c5 * (3. * s2 - 8. * s3 + 5. * s4) +
         Phi_r_c6 * (3. * s2 - 12. * s3 + 15 * s4 - 6. * s5) +
         Phi_r_c7 * (4. * s3 - 15. * s4 + 18. * s5 - 7. * s6)) /
        segwidth;
    ...
    double phiLup = sinqK * sinphiK * coslam - cosphiK * sinlam * sinalp -
                    cosqK * sinphiK * sinlam * cosalp;
    double phiLdown = sinqK * cosphiK * coslam + sinphiK * sinlam * sinalp -
                      cosqK * cosphiK * sinlam * cosalp;
    ...
    double beta;
    if (S_phys == 0.0 || lam < 1e-10 ||
        lam > (M_PI - 1e-10))  // Useful fix to make AAK match Kerr.
    {
      beta = 0.0;
    } else {
      // Be careful. I set beta = 0 due to division by zero in betadown.
      double betaup = -Sdotn + coslam * Ldotn;
      double betadown = sinqS * sin(phiK - phiS) * sinlam * cosalp +
                        (cosqK * Sdotn - cosqS) / sinqK * sinlam * sinalp;
    ...
    if (mich) {
      ...
      double phiw = orbphs + atan2(halfsqrt3 * cosqS + .5 * sinqS * cosorbphs,
                                   sinqS * sinorbphs);
      double psiup = .5 * cosqK - halfsqrt3 * sinqK * cos(orbphs - phiK) -
                     cosq * (cosqK * cosqS + sinqK * sinqS * cos(phiK - phiS));
      double psidown =
          .5 * sinqK * sinqS * sin(phiK - phiS) -
          halfsqrt3 * cos(orbphs) *
              (cosqK * sinqS * sin(phiS) - cosqS * sinqK * sin(phiK)) -
          halfsqrt3 * sin(orbphs) *
              (cosqS * sinqK * cos(phiK) - cosqK * sinqS * cos(phiS));
    ...
    d_RotCoeff(rot, n_rot, L_rot, S_rot, nxL_rot, nxS_rot, lam, qS, phiS, qK,
               phiK, alp);
...
void get_waveform(cmplx* waveform, double* interp_array, double M_phys,
                  double S_phys, double mu, double qS, double phiS, double qK,
                  double phiK, double dist, int nmodes, bool mich, int init_len,
                  int out_len, double delta_t, double* h_t) {
  ...
  for (int i = 0; i < number_of_old_spline_points - 1; i++) {
    ...
    make_waveform<<<gridDim, NUM_THREADS, 0, streams[i]>>>(
        waveform, interp_array, M_phys, S_phys, mu, qS, phiS, qK, phiK, dist,
        nmodes, mich, delta_t, h_t[i], i, start_inds[i], start_inds[i + 1],
        init_len, h_t[i + 1] - h_t[i]);
#else

    // CPU waveform generation
    make_waveform(waveform, interp_array, M_phys, S_phys, mu, qS, phiS, qK,
                  phiK, dist, nmodes, mich, delta_t, h_t[i], i, start_inds[i],
                  start_inds[i + 1], init_len, h_t[i + 1] - h_t[i]);
#endif
  }
