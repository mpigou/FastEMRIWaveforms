...
void fill_coefficients(int i, int length, double const* dydx, double dx,
                       double const* y, double* coeff1, double* coeff2,
                       double* coeff3) {
  const double slope = (y[i + 1] - y[i]) / dx;

  const double dydx_i = dydx[i];

  const double t = (dydx_i + dydx[i + 1] - 2 * slope) / dx;
...
CUDA_CALLABLE_MEMBER
void prep_splines(int i, int length, double* b, double* ud, double* diag,
                  double* ld, double const* x, double const* y) {
  // this performs a not-a-knot spline
  // need to adjust for ends of the splines
  if (i == length - 1) {
    const double dx1 = x[length - 2] - x[length - 3];
    const double dx2 = x[length - 1] - x[length - 2];
    const double d = x[length - 1] - x[length - 3];

    const double slope1 = (y[length - 2] - y[length - 3]) / dx1;
    const double slope2 = (y[length - 1] - y[length - 2]) / dx2;

    b[length - 1] = ((dx2 * dx2 * slope1 + (2 * d + dx2) * dx1 * slope2) / d);
    diag[length - 1] = dx1;
    ld[length - 1] = d;
    ud[length - 1] = 0.0;
  }
    ...
    const double dx1 = x[1] - x[0];
    const double dx2 = x[2] - x[1];
    const double d = x[2] - x[0];

    const double slope1 = (y[1] - y[0]) / dx1;
    const double slope2 = (y[2] - y[1]) / dx2;

    b[0] = ((dx1 + 2 * d) * dx2 * slope1 + dx1 * dx1 * slope2) / d;
    diag[0] = dx2;
    ud[0] = d;
    ld[0] = 0.0;
  }
    ...
    const double slope1 = (y[i] - y[i - 1]) / dx1;
    const double slope2 = (y[i + 1] - y[i]) / dx2;
...
// according to scipy Cubic Spline
CUDA_KERNEL
void fill_B(double* t_arr, double* y_all, double* B, double* upper_diag,
            double* diag, double* lower_diag, int ninterps, int length) {
#ifdef __CUDACC__
  const int start1 = blockIdx.y * blockDim.y + threadIdx.y;
  const int end1 = ninterps;
  const int diff1 = blockDim.y * gridDim.y;

  const int start2 = blockIdx.x * blockDim.x + threadIdx.x;
  const int end2 = length;
  const int diff2 = blockDim.x * gridDim.x;
#else
  const int start1 = 0;
  const int end1 = ninterps;
  const int diff1 = 1;
  ...
  for (int interp_i = start1; interp_i < end1;  // 2 for re and im
       interp_i += diff1) {
    for (int i = start2; i < end2; i += diff2) {
      int lead_ind = interp_i * length;
      prep_splines(i, length, &B[lead_ind], &upper_diag[lead_ind],
                   &diag[lead_ind], &lower_diag[lead_ind], &t_arr[lead_ind],
                   &y_all[interp_i * length]);
    }
  }
}
...
void fit_wrap(int m, int n, double* a, double* b, double* c, double* d_in) {
  ...
  CUSPARSE_CALL(cusparseDgtsv2StridedBatch_bufferSizeExt(
      handle, m, a, b, c, d_in, n, m, &bufferSizeInBytes));
  gpuErrchk(cudaMalloc(&pBuffer, bufferSizeInBytes));

  // solve banded matrix problem
  CUSPARSE_CALL(cusparseDgtsv2StridedBatch(handle, m,
                                           a,  // dl
                                           b,  // diag
                                           c,  // du
                                           d_in, n, m, pBuffer));
  ...
  for (int j = 0; j < n; j += 1) {
    int info = LAPACKE_dgtsv(LAPACK_COL_MAJOR, m, 1, &a[j * m + 1], &b[j * m],
                             &c[j * m], &d_in[j * m], m);
  }
...
CUDA_KERNEL
void fill_final_derivs(double* t_arr, double* interp_array, int ninterps,
                       int length) {
#ifdef __CUDACC__
...
void interpolate_arrays(double* t_arr, double* interp_array, int ninterps,
                        int length, double* B, double* upper_diag, double* diag,
                        double* lower_diag) {
  ...
  fill_B<<<gridDim, NUM_THREADS>>>(t_arr, interp_array, B, upper_diag, diag,
                                   lower_diag, ninterps, length);
  ...
  int num_blocks_fill_derivs =
      std::ceil((ninterps + NUM_THREADS - 1) / NUM_THREADS);

  fill_final_derivs<<<num_blocks_fill_derivs, NUM_THREADS>>>(
      t_arr, interp_array, ninterps, length);
  cudaDeviceSynchronize();
  gpuErrchk(cudaGetLastError());

#else

  fill_B(t_arr, interp_array, B, upper_diag, diag, lower_diag, ninterps,
         length);

  fit_wrap(length, ninterps, lower_diag, diag, upper_diag, B);

  set_spline_constants(t_arr, interp_array, B, ninterps, length);
...
CUDA_KERNEL
void interp_time_for_fd(double* output, double* t_arr, double* tstar,
                        int* ind_tstar, double* interp_array, int ninterps,
                        int length, bool* run) {
...
void interp_time_for_fd_wrap(double* output, double* t_arr, double* tstar,
                             int* ind_tstar, double* interp_array, int ninterps,
                             int length, bool* run) {
  int num_modes = int((ninterps - 4) / 2.);
#ifdef __CUDACC__
  ...
  interp_time_for_fd<<<num_blocks, NUM_THREADS>>>(
      output, t_arr, tstar, ind_tstar, interp_array, ninterps, length, run);
  cudaDeviceSynchronize();
  gpuErrchk(cudaGetLastError());

#else
  interp_time_for_fd(output, t_arr, tstar, ind_tstar, interp_array, ninterps,
                     length, run);
...
// this uses an efficient summation by loading mode information into shared
// memory shared memory is leveraged heavily
CUDA_KERNEL
void make_waveform(cmplx* waveform, double* interp_array,
                   double* phase_interp_coeffs, int* m_arr_in, int* k_arr_in,
                   int* n_arr_in, int num_teuk_modes, cmplx* Ylms_in,
                   double delta_t, double start_t, int old_ind, int start_ind,
                   int end_ind, int init_length, double phase_interp_t_here,
                   double phase_interp_segwidth) {
  // int num_pars = 2;
  cmplx trans(0.0, 0.0);
  ...
  double Phi_phi_i, Phi_theta_i, Phi_r_i, t, x, x2, x3, mode_val_re,
      mode_val_im, s, s1;
  ...
  if (true) {
    ...
    // fill phase values. These will be same for all modes
    //   int ind_Phi_phi = old_ind * (2 * num_teuk_modes + num_pars) +
    //   num_teuk_modes * 2 + 0; int ind_Phi_r = old_ind * (2 * num_teuk_modes +
    //   num_pars) + num_teuk_modes * 2 + 1;
  ...
  // this does a special loop to fill mode information into shared memory in
  // chunks
  for (int block_y = 0; block_y < num_breaks; block_y += 1) {
    num_teuk_here = (((block_y + 1) * MAX_MODES_BLOCK) <= num_teuk_modes)
                        ? MAX_MODES_BLOCK
    ...
    for (int i = start; i < end; i += diff) {
      ...
      // int ind_re = old_ind * (2 * num_teuk_modes + num_pars) + (init_ind +
      // i); int ind_im = old_ind * (2 * num_teuk_modes + num_pars) +
      // num_teuk_modes + (init_ind + i);
      int ind_re = old_ind * (2 * num_teuk_modes) + (init_ind + i);
      int ind_im =
          old_ind * (2 * num_teuk_modes) + num_teuk_modes + (init_ind + i);
    ...
    for (int i = start; i < end; i += diff) {
      ...
      // printf("%d %f %f %f %f %f %f %f %f \n", old_ind, s, pp_y, pp_c1, pp_c2,
      // pp_c3, pp_c4, pp_c5, pp_c6, pp_c7);
      ...
      for (int j = 0; j < num_teuk_here; j += 1) {
        ...
        // mode_val_re = mode_re_y[j] + mode_re_c1[j] * x + mode_re_c2[j] * x2 +
        // mode_re_c3[j] * x3; mode_val_im = mode_im_y[j] + mode_im_c1[j] * x +
        // mode_im_c2[j] * x2 + mode_im_c3[j] * x3; mode_val = mode_val_re +
        // complexI * mode_val_im;

        // fod phase = m * Phi_phi_i + n * Phi_r_i;
        // partial_mode = mode_val * gcmplx::exp(minus_I * phase);
        partial_mode = gcmplx::exp(minus_I * (m * Phi_phi_i + k * Phi_theta_i +
                                              n * Phi_r_i)) *
                       (mode_re_y[j] + mode_re_c1[j] * x + mode_re_c2[j] * x2 +
                        mode_re_c3[j] * x3 +
                        complexI * (mode_im_y[j] + mode_im_c1[j] * x +
                                    mode_im_c2[j] * x2 + mode_im_c3[j] * x3));
        ...
        if (m != 0) {
          ...
          trans_minus_m = gcmplx::conj(partial_mode) *
                          Ylm_minus_m;  // conjugate is distributive, so apply
                                        // to the product
        } else
...
// with uneven spacing in t in the sparse arrays, need to determine which
// timesteps the dense arrays fall into for interpolation effectively the
// boundaries and length of each interpolation segment of the dense array in the
// sparse array
void find_start_inds(int start_inds[], int unit_length[], double* t_arr,
                     double delta_t, int* length, int new_length) {
...
void get_waveform(cmplx* d_waveform, double* interp_array,
                  double* phase_interp_t, double* phase_interp_coeffs, int* d_m,
                  int* d_k, int* d_n, int init_len, int out_len,
  ...
  for (int i = 0; i < number_of_old_spline_points - 1; i++) {
    ...
    int num_blocks =
        std::ceil((unit_length[i] + NUM_THREADS - 1) / NUM_THREADS);
    ...
    make_waveform<<<gridDim, NUM_THREADS, 0, streams[i]>>>(
        d_waveform, interp_array, phase_interp_coeffs, d_m, d_k, d_n,
        num_teuk_modes, d_Ylms, delta_t, h_t[i], i, start_inds[i],
        start_inds[i + 1], init_len, phase_interp_t[i],
        phase_interp_t[i + 1] - phase_interp_t[i]);
    cudaDeviceSynchronize();
    gpuErrchk(cudaGetLastError());
    cudaDeviceSynchronize();
#else
    ...
    make_waveform(d_waveform, interp_array, phase_interp_coeffs, d_m, d_k, d_n,
                  num_teuk_modes, d_Ylms, delta_t, h_t[i], i, start_inds[i],
                  start_inds[i + 1], init_len, phase_interp_t[i],
                  phase_interp_t[i + 1] - phase_interp_t[i]);
#endif
...
CUDA_CALLABLE_MEMBER
double tgamma1pm1(double dz) { return tgamma(dz + 1.) - 1.; }
...
int temme_ik(double v, cmplx x, cmplx* K, cmplx* K1) {
  ...
  c = abs(v) < epsilon ? 1.0 : sin(M_PI * v) / (v * M_PI);
  d = abs(sigma) < epsilon ? 1.0 : sinh(sigma) / sigma;
  gamma1 = abs(cmplx(v, 0.0)) < epsilon ? -euler : (0.5 / v) * (gp - gm) * c;
  gamma2 = (2. + gp + gm) * c / 2.;
...
int CF2_ik(double v, cmplx x, cmplx* Kv, cmplx* Kv1) {
  ...
  for (k = 2; k < max_series_iterations; k++)  // starting from 2
  {
    ...
    // test case occurs when x is close to 2, for example
    // cyl_bessel_k(9.125, 2.125).
    //
    if (abs(q) < epsilon) {
      C *= q;
      prev /= q;
...
cmplx SPAFunc(const double x) {
  //$x = (2\pi/3)\dot f^3/\ddot f^2$ and spafunc is $i \sqrt{x} e^{-i x}
  // K_{1/3}(-i x)$.
  cmplx II(0.0, 1.0);
  cmplx ans;
  ...
  if (abs(x) <= 7.) {
    ...
    const cmplx tmp_p = pow(xx, 1. / 6.);  // cbrt(sqrt(x));//
    const cmplx pref1 = gcmplx::exp(-2. * M_PI * II / 3.) * tmp_p * tmp_p *
                        tmp_p * tmp_p * tmp_p * Gamm13 /
                        1.2599210498948732;  // pow(2., 1./3.);
    const cmplx pref2 = gcmplx::exp(-M_PI * II / 3.) * tmp_p * Gamp13 /
                        1.5874010519681994748;  // pow(2., 2./3.);
    const double x2 = x * x;

    const double c1_0 = 0.5, c1_2 = -0.09375, c1_4 = 0.0050223214285714285714;
    const double c1_6 = -0.00012555803571428571429,
                 c1_8 = 1.8109332074175824176e-6;
    const double c1_10 = -1.6977498819539835165e-8,
                 c1_12 = 1.1169407118118312608e-10;
    const double c1_14 = -5.4396463237589184781e-13,
                 c1_16 = 2.0398673714095944293e-15;
    const double c1_18 = -6.0710338434809358015e-18,
                 c1_20 = 1.4687985105195812423e-20;
    const double c1_22 = -2.9454515585285720100e-23,
                 c1_24 = 4.9754249299469121790e-26;
    ...
    const double c2_0 = 1., c2_2 = -0.375, c2_4 = 0.028125,
                 c2_6 = -0.00087890625;
    const double c2_8 = 0.000014981356534090909091,
                 c2_10 = -1.6051453429383116883e-7;
    const double c2_12 = 1.1802539286311115355e-9,
                 c2_14 = -6.3227889033809546546e-12;
    const double c2_16 = 2.5772237377911499951e-14,
                 c2_18 = -8.2603324929203525483e-17;
    const double c2_20 = 2.1362928861000911763e-19,
                 c2_22 = -4.5517604107246260858e-22;
    const double c2_24 = 8.1281435905796894390e-25,
                 c2_26 = -1.2340298973552160079e-27;
    ...
    const cmplx c_0 = II, c_1 = 0.069444444444444444444,
                c_2 = -0.037133487654320987654 * II;
    const cmplx c_3 = -0.037993059127800640146,
                c_4 = 0.057649190412669721333 * II;
    const cmplx c_5 = 0.11609906402551541102,
                c_6 = -0.29159139923075051147 * II;
...
// build mode value with specific phase and amplitude values; mode indexes; and
// spherical harmonics CUDA_CALLABLE_MEMBER cmplx get_mode_value_fd(double t,
// double f, double fdot, double fddot, cmplx amp_term1, double phase_term,
// cmplx Ylm){
//     cmplx I(0.0, 1.0);

//     // Waveform Amplitudes
//     // $x = (2\pi/3)\dot f^3/\ddot f^2$ and spafunc is $i \sqrt{x} e^{-i x}
//     K_{1/3}(-i x)$. double arg = 2.* PI * pow(fdot, 3) / (3.* pow(fddot, 2));
//     cmplx amp_term2 = -1.0 * fdot/abs(fddot) * 2./sqrt(3.) * SPAFunc(arg) /
//     gcmplx::sqrt(cmplx(arg, 0.0));
//     //cmplx amp_term2 = 0.0;
...
CUDA_CALLABLE_MEMBER
void cube_roots(cmplx* r1o, cmplx* r2o, cmplx* r3o, double a, double b,
                double c, double d, bool check) {
  double b2 = b * b;
  double b3 = b2 * b;
  double a2 = a * a;
  ...
  cmplx C =
      pow((Delta_1 +
           sqrt(cmplx(Delta_1 * Delta_1 - 4 * (Delta_0 * Delta_0 * Delta_0),
                      0.0))) /
              2.,
          1. / 3.);
...
CUDA_KERNEL
void make_generic_kerr_waveform_fd(
    cmplx* waveform, double* interp_array, double* phase_interp_coeffs,
    double* phase_interp_t, int* m_arr_in, int* k_arr_in, int* n_arr_in,
    int num_teuk_modes, double delta_t, double* old_time_arr, int init_length,
    int data_length, double* frequencies, int* seg_start_inds,
    int* seg_end_inds, int num_segments, cmplx* Ylm_all, int zero_index,
  ...
  for (int mode_i = start2; mode_i < num_teuk_modes; mode_i += diff2) {
    ...
    for (int seg_i = start3; seg_i < num_segments; seg_i += diff3) {
      ...
      Phi_theta_y =
          phase_interp_coeffs[0 * num_phase_spline + init_length - 1 + seg_i];
      Phi_theta_c1 =
          phase_interp_coeffs[1 * num_phase_spline + init_length - 1 + seg_i];
      Phi_theta_c2 =
          phase_interp_coeffs[2 * num_phase_spline + init_length - 1 + seg_i];
      Phi_theta_c3 =
          phase_interp_coeffs[3 * num_phase_spline + init_length - 1 + seg_i];
      Phi_theta_c4 =
          phase_interp_coeffs[4 * num_phase_spline + init_length - 1 + seg_i];
      Phi_theta_c5 =
          phase_interp_coeffs[5 * num_phase_spline + init_length - 1 + seg_i];
      Phi_theta_c6 =
          phase_interp_coeffs[6 * num_phase_spline + init_length - 1 + seg_i];
      Phi_theta_c7 =
          phase_interp_coeffs[7 * num_phase_spline + init_length - 1 + seg_i];

      Phi_r_y = phase_interp_coeffs[0 * num_phase_spline + 2 * init_length - 2 +
                                    seg_i];
      Phi_r_c1 = phase_interp_coeffs[1 * num_phase_spline + 2 * init_length -
                                     2 + seg_i];
      Phi_r_c2 = phase_interp_coeffs[2 * num_phase_spline + 2 * init_length -
                                     2 + seg_i];
      Phi_r_c3 = phase_interp_coeffs[3 * num_phase_spline + 2 * init_length -
                                     2 + seg_i];
      Phi_r_c4 = phase_interp_coeffs[4 * num_phase_spline + 2 * init_length -
                                     2 + seg_i];
      Phi_r_c5 = phase_interp_coeffs[5 * num_phase_spline + 2 * init_length -
                                     2 + seg_i];
      Phi_r_c6 = phase_interp_coeffs[6 * num_phase_spline + 2 * init_length -
                                     2 + seg_i];
      Phi_r_c7 = phase_interp_coeffs[7 * num_phase_spline + 2 * init_length -
                                     2 + seg_i];
      ...
      //     y_ind, num_base, num_teuk_modes, mode_i, init_length);
...
// TODO: cleanup registers for CUDA without OMP
      ...
      int diff = 1;
#endif
      for (int i = start + seg_start_ind_here; i <= seg_end_ind_here;
           i += diff) {
        int ind_f = i;
        ...
        roots[1] = root2.real();
        roots[2] = root3.real();
        // if ((f_y - f > 0.0)) printf("roots: %.18e %.18e %.18e %.18e %.18e
        // %.18e %.18e %.18e %.18e %.18e %.18e %.18e %d %d %d\n", root1, root2,
        // root3, f_c3, f_c2, f_c1, f_y - f, f_y, f_y2, f, start_t, end_t,
        // ind_i, m, n); if ((f_y - f > 0.0)) printf("roots: %d %d %d %d %.12e
        // %.12e %.12e %d %d\n", ind_i, m, n, (mode_i * max_length + i) * 2 +
        // which, f,f_y, f_y2, mode_start_ind_here, i);

        // if ((m == 2) && (seg_i == 80) && (i == seg_start_ind_here + 10))
        // printf("roots: i: %d seg_i: %d m: %d n: %d root1: %.12e\n t_s=
        // %.12e\nt_f= %.12e \nf= %.12e \nf_phi_y= %.12e \nf_r_y= %.12e
        // \nf_phi_c1= %.12e \nf_r_c1= %.12e \nf_phi_c2= %.12e \nf_r_c2= %.12e
        // \nf_phi_c3= %.12e \nf_r_c3= %.12e\n", i, seg_i, m, n, roots[0],
        // old_time_start, old_time_end, f, f_phi_y, f_r_y, f_phi_c1, f_r_c1,
        // f_phi_c2, f_r_c2, f_phi_c3, f_r_c3);
        // if ((mode_i == 0) && (seg_i == 6)) printf("roots: %d %d %d %.12e
        // %.12e %.12e\n", i, m, n, f_phi_y, f_r_y, f_y);

        // if ((i < 10) && (mode_i < 3)) printf("root check: %d %d %.18e %.18e
        // %.18e %.18e %.18e \n", mode_i, i, root1, root2, root3, start_t,
        // end_t); if ((i < 10) && (mode_i < 3)) printf("root check: %d %d %.18e
        // %.18e %.18e %.18e %.18e %.18e \n", mode_i, i, f_c3, f_c2, f_c1, (f_y
        // - f), f_y, f);
        ...
        for (int root_i = 0; root_i < 3; root_i += 1) {
          ...
          if ((t < old_time_end) && (t >= old_time_start)) {
            ...
            double R_mode_re = R_mode_re_y + R_mode_re_c1 * x +
                               R_mode_re_c2 * x2 + R_mode_re_c3 * x3;
            double R_mode_im = R_mode_im_y + R_mode_im_c1 * x +
                               R_mode_im_c2 * x2 + R_mode_im_c3 * x3;
            // double L_mode_re = L_mode_re_y + L_mode_re_c1 * x + L_mode_re_c2
            // * x2  + L_mode_re_c3 * x3; double L_mode_im = L_mode_im_y +
            // L_mode_im_c1 * x + L_mode_im_c2 * x2  + L_mode_im_c3 * x3;
                ...
                s * (Phi_r_c1 +
                     s1 * (Phi_r_c2 +
                           s * (Phi_r_c3 +
                                s1 * (Phi_r_c4 +
                                      s * (Phi_r_c5 +
                                           s1 * (Phi_r_c6 + s * Phi_r_c7))))));

            double fdot_phi_i =
                (-2. * Phi_phi_c2 + Phi_phi_c3 * (2. - 6. * s) +
                 Phi_phi_c4 * (2. - 12. * s + 12. * s2) +
                 Phi_phi_c5 * (6. * s - 24. * s2 + 20. * s3) +
                 Phi_phi_c6 * (6. * s - 36. * s2 + 60. * s3 - 30. * s4) +
                 Phi_phi_c7 * (12. * s2 - 60. * s3 + 90. * s4 - 42. * s5)) /
                (2. * M_PI * segwidth2);
            double fdot_theta_i =
                (-2. * Phi_theta_c2 + Phi_theta_c3 * (2. - 6. * s) +
                 Phi_theta_c4 * (2. - 12. * s + 12. * s2) +
                 Phi_theta_c5 * (6. * s - 24. * s2 + 20. * s3) +
                 Phi_theta_c6 * (6. * s - 36. * s2 + 60. * s3 - 30. * s4) +
                 Phi_theta_c7 * (12. * s2 - 60. * s3 + 90. * s4 - 42. * s5)) /
                (2. * M_PI * segwidth2);
            double fdot_r_i =
                (-2. * Phi_r_c2 + Phi_r_c3 * (2. - 6. * s) +
                 Phi_r_c4 * (2. - 12. * s + 12. * s2) +
                 Phi_r_c5 * (6. * s - 24. * s2 + 20. * s3) +
                 Phi_r_c6 * (6. * s - 36. * s2 + 60. * s3 - 30. * s4) +
                 Phi_r_c7 * (12. * s2 - 60. * s3 + 90. * s4 - 42. * s5)) /
                (2. * M_PI * segwidth2);

            double fddot_phi_i =
                (-6. * Phi_phi_c3 + Phi_phi_c4 * (-12. + 24. * s) +
                 Phi_phi_c5 * (6. - 48. * s + 60. * s2) +
                 Phi_phi_c6 * (6. - 72. * s + 180. * s2 - 120. * s3) +
                 Phi_phi_c7 * (24. * s - 180. * s2 + 360. * s3 - 210. * s4)) /
                (2. * M_PI * segwidth3);
            double fddot_theta_i =
                (-6. * Phi_theta_c3 + Phi_theta_c4 * (-12. + 24. * s) +
                 Phi_theta_c5 * (6. - 48. * s + 60. * s2) +
                 Phi_theta_c6 * (6. - 72. * s + 180. * s2 - 120. * s3) +
                 Phi_theta_c7 * (24. * s - 180. * s2 + 360. * s3 - 210. * s4)) /
                (2. * M_PI * segwidth3);
            double fddot_r_i =
                (-6. * Phi_r_c3 + Phi_r_c4 * (-12. + 24. * s) +
                 Phi_r_c5 * (6. - 48. * s + 60. * s2) +
                 Phi_r_c6 * (6. - 72. * s + 180. * s2 - 120. * s3) +
                 Phi_r_c7 * (24. * s - 180. * s2 + 360. * s3 - 210. * s4)) /
                (2. * M_PI * segwidth3);
            ...
            // if (i == 1653451) printf("%d %d %d %.12e %.12e %.12e %.12e %.12e
            // %.12e %.12e %.12e %.12e\n", i, m, n, t, f, fdot, fddot,
            // R_amp.real(), R_amp.imag(), phase_term, fddot_phi_i, fddot_r_i);
            // if (i == 1653451) printf("%d %d %d %.12e %.12e %.12e %.12e %.12e
            // %.12e %.12e\n", i, m, n, t, f, fdot, fddot, R_amp.real(),
            // R_amp.imag(), phase_term);

            // store some intermediate quantities to save some function
            // evaluations
            ...
            // then i\sqrt{-x} e^{-i -x} K_{1/3}(-i -x) -> conj(i * (-i) *
            // \sqrt{x} e^{-i x} K_{1/3}(-i x))

            // 1 / sqrt(-x) -> 1 / i sqrt(x) under x -> -x, which cancels with
            // the i from the first sqrt so: amp_term2(-x) = conj(-1.0 *
            // -fdot/abs(fddot) * 2./sqrt(3.) * i\sqrt{x} e^{-i x} K_{1/3}(-i
            // x)) / sqrt(-x)
            //               = conj(-1.0 * -fdot/abs(fddot) * 2./sqrt(3.) * i *
            //               (-i) * \sqrt{x} e^{-i x} K_{1/3}(-i x)) / (i
            //               sqrt(x)) = conj(-1.0 * fdot/abs(fddot)
            //               * 2./sqrt(3.) * i\sqrt{x} e^{-i x} K_{1/3}(-i x) /
            //               sqrt(x)) = conj(amp_term2(x))

            double temp_arg =
                2. * M_PI * fdot * fdot * fdot / (3. * fddot * fddot);
            cmplx amp_term2 = -1.0 * fdot / abs(fddot) * 2. / sqrt(3.) *
                              SPAFunc(temp_arg) /
                              gcmplx::sqrt(cmplx(temp_arg, 0.0));

            cmplx temp_exp =
                R_amp * amp_term2 *
                gcmplx::exp(complexI * (2. * M_PI * f * t - phase_term));

            cmplx R_tmp =
                Ylm_plus_m * temp_exp;  // combine with spherical harmonic
            ...
            if ((m != 0.0) && (include_minus_m)) {
              L_tmp =
                  Ylm_minus_m *
                  gcmplx::conj(
                      temp_exp);  // as above, take the conjugate and we're done
            }

            // if (m + k + n != 0)
            //{
            // cmplx L_tmp = get_mode_value_generic(L_amp, Phi_phi_i, Phi_r_i,
            // Phi_theta_i, -m, -k, -n);
            // }
...
// fill waveform
            ...
            // cmplx R_tmp = get_mode_value_generic(R_amp, Phi_phi_i, Phi_r_i,
            // Phi_theta_i, m, k, n);
          }
        }
      }
...
// function for building interpolated EMRI waveform from python
void get_waveform_generic_fd(
    cmplx* waveform, double* interp_array, double* phase_interp_t,
    double* phase_interp_coeffs, int* m_arr_in, int* k_arr_in, int* n_arr_in,
    int num_teuk_modes, double delta_t, double* old_time_arr, int init_length,
    int data_length, double* frequencies, int* seg_start_inds,
    int* seg_end_inds, int num_segments, cmplx* Ylm_all, int zero_index,
    bool include_minus_m, bool separate_modes) {
// int NUM_THREADS = 256;
  ...
  // launch one worker kernel per stream
  make_generic_kerr_waveform_fd<<<gridDim, NUM_THREADS_FD>>>(
      waveform, interp_array, phase_interp_coeffs, phase_interp_t, m_arr_in,
      k_arr_in, n_arr_in, num_teuk_modes, delta_t, old_time_arr, init_length,
      data_length, frequencies, seg_start_inds, seg_end_inds, num_segments,
