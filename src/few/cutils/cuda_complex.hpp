...
CUDA_CALLABLE_MEMBER complex<_Tp> operator*(const complex<_Tp>& __z,
  ...
  if (isnan(__x) && isnan(__y)) {
    ...
    if (std::isinf(__a) || isinf(__b)) {
      ...
      if (isnan(__c)) __c = copysign(_Tp(0), __c);
      if (isnan(__d)) __d = copysign(_Tp(0), __d);
      __recalc = true;
    }
    if (std::isinf(__c) || isinf(__d)) {
      __c = copysign(isinf(__c) ? _Tp(1) : _Tp(0), __c);
      __d = copysign(isinf(__d) ? _Tp(1) : _Tp(0), __d);
      if (isnan(__a)) __a = copysign(_Tp(0), __a);
      if (isnan(__b)) __b = copysign(_Tp(0), __b);
      __recalc = true;
    }
    if (!__recalc &&
...
CUDA_CALLABLE_MEMBER complex<_Tp> operator/(const complex<_Tp>& __z,
  ...
  if (isnan(__x) && isnan(__y)) {
    ...
    } else if ((isinf(__a) || isinf(__b)) && std::isfinite(__c) &&
               std::isfinite(__d)) {
      __a = copysign(isinf(__a) ? _Tp(1) : _Tp(0), __a);
      __b = copysign(isinf(__b) ? _Tp(1) : _Tp(0), __b);
      __x = _Tp(INFINITY) * (__a * __c + __b * __d);
      __y = _Tp(INFINITY) * (__b * __c - __a * __d);
    } else if (std::isinf(__logbw) && __logbw > _Tp(0) && std::isfinite(__a) &&
               std::isfinite(__b)) {
      __c = copysign(isinf(__c) ? _Tp(1) : _Tp(0), __c);
      __d = copysign(isinf(__d) ? _Tp(1) : _Tp(0), __d);
...
#pragma warning(suppress: 4756) // Ignore INFINITY related warning
        ...
        else if (std::isinf(__logbw) && __logbw > float(0) && std::isfinite(__a)
&& std::isfinite(__b))
        {
...
inline CUDA_CALLABLE_MEMBER _Tp norm(const complex<_Tp>& __c) {
  if (std::isinf(__c.real())) return fabs(__c.real());
  if (std::isinf(__c.imag())) return fabs(__c.imag());
...
inline CUDA_CALLABLE_MEMBER complex<double> proj(double __re) {
  if (std::isinf(__re)) __re = fabs(__re);
  return complex<double>(__re);
}
...
CUDA_CALLABLE_MEMBER complex<_Tp> polar(const _Tp& __rho,
  ...
  if (isnan(__rho) || signbit(__rho)) return complex<_Tp>(_Tp(NAN), _Tp(NAN));
  if (isnan(__theta)) {
    if (std::isinf(__rho)) return complex<_Tp>(__rho, __theta);
    return complex<_Tp>(__theta, __theta);
  }
  if (std::isinf(__theta)) {
    if (std::isinf(__rho)) return complex<_Tp>(__rho, _Tp(NAN));
...
CUDA_CALLABLE_MEMBER complex<_Tp> sqrt(const complex<_Tp>& __x) {
  if (std::isinf(__x.imag())) return complex<_Tp>(_Tp(INFINITY), __x.imag());
...
CUDA_CALLABLE_MEMBER complex<_Tp> exp(const complex<_Tp>& __x) {
  ...
  if (std::isinf(__x.real())) {
    if (__x.real() < _Tp(0)) {
      if (!std::isfinite(__i)) __i = _Tp(1);
    } else if (__i == 0 || !std::isfinite(__i)) {
      if (std::isinf(__i)) __i = _Tp(NAN);
...
CUDA_CALLABLE_MEMBER complex<_Tp> asinh(const complex<_Tp>& __x) {
  ...
  if (std::isinf(__x.real())) {
    if (isnan(__x.imag())) return __x;
    if (std::isinf(__x.imag()))
      return complex<_Tp>(__x.real(), copysign(__pi * _Tp(0.25), __x.imag()));
    return complex<_Tp>(__x.real(), copysign(_Tp(0), __x.imag()));
  }
  if (isnan(__x.real())) {
    if (std::isinf(__x.imag())) return complex<_Tp>(__x.imag(), __x.real());
    if (__x.imag() == 0) return __x;
...
CUDA_CALLABLE_MEMBER complex<_Tp> acosh(const complex<_Tp>& __x) {
  const _Tp __pi(atan2(+0., -0.));
  if (std::isinf(__x.real())) {
    if (isnan(__x.imag())) return complex<_Tp>(fabs(__x.real()), __x.imag());
    if (std::isinf(__x.imag()))
      if (__x.real() > 0)
        return complex<_Tp>(__x.real(), copysign(__pi * _Tp(0.25), __x.imag()));
      else
...
CUDA_CALLABLE_MEMBER complex<_Tp> sinh(const complex<_Tp>& __x) {
  ...
  if (__x.real() == 0 && !std::isfinite(__x.imag()))
    return complex<_Tp>(__x.real(), _Tp(NAN));
  if (__x.imag() == 0 && !std::isfinite(__x.real())) return __x;
...
CUDA_CALLABLE_MEMBER complex<_Tp> tanh(const complex<_Tp>& __x) {
  if (std::isinf(__x.real())) {
    if (!std::isfinite(__x.imag())) return complex<_Tp>(_Tp(1), _Tp(0));
    return complex<_Tp>(_Tp(1),
                        copysign(_Tp(0), std::sin(_Tp(2) * __x.imag())));
  }
  if (isnan(__x.real()) && __x.imag() == 0) return __x;
  _Tp __2r(_Tp(2) * __x.real());
  _Tp __2i(_Tp(2) * __x.imag());
  _Tp __d(cosh(__2r) + std::cos(__2i));
...
CUDA_CALLABLE_MEMBER complex<_Tp> acos(const complex<_Tp>& __x) {
  const _Tp __pi(atan2(+0., -0.));
  if (std::isinf(__x.real())) {
    if (isnan(__x.imag())) return complex<_Tp>(__x.imag(), __x.real());
    if (std::isinf(__x.imag())) {
      if (__x.real() < _Tp(0))
  ...
  if (isnan(__x.real())) {
    if (std::isinf(__x.imag())) return complex<_Tp>(__x.real(), -__x.imag());
    return complex<_Tp>(__x.real(), __x.real());
  }
  if (std::isinf(__x.imag())) return complex<_Tp>(__pi / _Tp(2), -__x.imag());
  if (__x.real() == 0) return complex<_Tp>(__pi / _Tp(2), -__x.imag());
  complex<_Tp> __z = log(__x + sqrt(pow(__x, _Tp(2)) - _Tp(1)));
  if (signbit(__x.imag()))
