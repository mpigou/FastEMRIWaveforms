...
FEW_INLINE CUDA_CALLABLE_MEMBER void fpbspl(const double* t, int k, double x,
                                            int l, double* h) {
...
void fpbisp(double* z, const double* tx, int nx, const double* ty, int ny,
            double* c, int kx, int ky, const double x, int mx, const double y,
            int my) {
  ...
  // printf("%d %d %d %d %e %e %e %e %e %e %d\n", i, l, ky1, l - ky1, arg, y[i],
  // tb, te, ty_shared[l], ty_shared[nky1], nky1);
  ly = l - ky1;
  // for (j = 0; j < ky1; ++j)
  //   wy[i * ky1 + j] = h[j];
  ...
  for (i1 = 0; i1 < kx1; i1++) {
    l2 = l1;
    for (j1 = 0; j1 < ky1; j1++) sp += c[l2++] * wx[i1] * wy[j1];
    l1 += nky1;
  }
  *z = sp;
  //}
...

{
  ...
  for (int c_i = start_block; c_i < num_indiv_c; c_i += block_inc) {
    ...
    for (int i = full_loop_start; i < mx; i += full_loop_inc) {
      fpbisp(&z_temp, tx_shared, nx, ty_shared, ny, c_indiv, kx, ky, x[i], 1,
             y[i], 1);
...
void interp2D_wrap(double* z, const double* tx, int nx, const double* ty,
                   int ny, double* c, int kx, int ky, const double* x, int mx,
                   const double* y, int my, int num_indiv_c, int len_indiv_c) {
  if (mx != my) {
  ...
  auto shared_memory_size =
      nx * sizeof(double) + ny * sizeof(double) + len_indiv_c * sizeof(double);

  gpuErrchk(cudaFuncSetAttribute(interp2D,
  ...
  interp2D<<<grid, NUM_THREADS, shared_memory_size>>>(
      z, tx, nx, ty, ny, c, kx, ky, x, mx, y, my, num_indiv_c, len_indiv_c);
  gpuErrchk(cudaPeekAtLastError());
  gpuErrchk(cudaDeviceSynchronize());
#else
  interp2D(z, tx, nx, ty, ny, c, kx, ky, x, mx, y, my, num_indiv_c,
           len_indiv_c);
